...
from tkinter import messagebox  # Thư viện cho các hộp thoại thông báo
...
def get_valid_value(key, min_val, max_val, default_value):
    while True:
        try:
            value = float(input(f"{key} (từ {min_val} đến {max_val}, mặc định {default_value}): "))# Yêu cầu nhập giá trị từ người dùng
            if min_val <= value <= max_val: # Kiểm tra giá trị nhập vào có hợp lệ không
                return value
            else:
                print(f"Giá trị phải nằm trong khoảng từ {min_val} đến {max_val}. Hãy nhập lại.")# Thông báo lỗi nếu giá trị không hợp lệ
        except ValueError:  # Xử lý khi người dùng nhập dữ liệu không hợp lệ
            print("Giá trị không hợp lệ! Hãy nhập lại.")# Thông báo lỗi khi nhập không phải số
    ...
    for key, value in user_data.items():
        try:
            ...
            if min_val <= num_value <= max_val:
                ...
                error_labels[key].config(text=f"{key} vượt quá giới hạn cho phép!")# Thông báo lỗi nếu giá trị vượt quá giới hạn
                error_found = True
                cleaned_data[key] = default_values[key]# Gán giá trị mặc định nếu có lỗi
        except ValueError:# Xử lý khi giá trị không hợp lệ
            error_labels[key].config(text=f"{key} không hợp lệ! Sử dụng giá trị mặc định.")# Thông báo lỗi nếu không phải số
            error_found = True
            cleaned_data[key] = default_values[key]# Gán giá trị mặc định nếu có lỗi
...
def loadData(filename="ThoiTiet_dulieu.csv"):
    ...
    with open(path) as f:
        data = np.array(list(csv.reader(f))[1:]) # Đọc dữ liệu từ tệp và bỏ qua dòng tiêu đề
...
def calcDistance(pointA, pointB):
    return np.linalg.norm(pointA[:5].astype(float) - pointB[:5].astype(float))# Tính khoảng cách Euclid

# Thuật toán k-NN
def kNearestNeighbor(trainSet, point, k=19):
    distances = [{"label": item[-1], "value": calcDistance(item, point)} for item in trainSet]# Tính khoảng cách và gán nhãn
    return [item["label"] for item in sorted(distances, key=lambda x: x["value"])[:k]] # Trả về k nhãn gần nhất
...
def load_icon(prediction):
    ...
    icon_path = get_relative_path(icons.get(prediction, "default_icon.png")) # Lấy đường dẫn đến icon dựa trên dự đoán
...
def create_gui():
    ...
    for i, (label, unit)  in enumerate(zip(labels, units)):
        ...
        tk.Label(form_frame, text=f"{label} {unit}").grid(row=i*2, column=0, sticky='w', pady=5) # Tạo nhãn cho mỗi trường
        
        # Tạo ô nhập liệu tương ứng cho từng nhãn
        entry = tk.Entry(form_frame, font=("Arial", 12))   # Tạo ô nhập liệu cho mỗi trường
      ...
      # Tạo nhãn lỗi, ban đầu để trống
        ...
        error_label.grid(row=i*2+1, column=1, pady=5, sticky='w') # Đặt nhãn lỗi ngay dưới ô nhập liệu
    ...
    def submit_form():
        ...
        if error_found: # Nếu có lỗi
            result_label.config(text="Dữ liệu không hợp lệ! Vui lòng kiểm tra các trường có lỗi.")
        ...
        input_path = get_relative_path("ThoiTiet_input.csv") # Lấy đường dẫn tương đối của file
        ...
        translated_prediction = label_translation.get(prediction, prediction)  # Dịch nhãn sang tiếng Việt nếu cần

        # Hiển thị kết quả dự đoán trong giao diện
        result_label.config(text=f"Dự đoán: {translated_prediction}", font=("Arial", 14, "bold"))
        ...
        with open(get_relative_path("ThoiTiet_testInput.csv"), "w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow([label_translation.get(item[0][-1], item[0][-1]), translated_prediction]) # Ghi dữ liệu

        # Tính toán và hiển thị độ chính xác
        accuracy = calculate_accuracy(testSet, trainSet, k=5) # Tính tỷ lệ chính xác
        accuracy_label.config(text=f"Tỷ lệ chính xác: {accuracy:.2f}%") # Hiển thị tỷ lệ chính xác

   # Nút "Dự Đoán" (được căn giữa và có màu sắc tùy chỉnh)
    submit_button = tk.Button(root, text="Dự Đoán", command=submit_form, bg="green", fg="white", font=("Arial", 12))
